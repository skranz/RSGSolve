  ...
  try
    {
      ...
      //double points[numStates][numPoints][numPlayers];
      //vector<vector<vector<double> > >
      //  points (numStates,vector<vector<double> >(numPoints,vector <double>(numPlayers,0)));
      ...
      while (it->getRevolution() == numRevolutions) {
        ...
        for (int state = 0; state < numStates; state++) {
          ...
          for (int player = 0; player < numPlayers; player++) {
            ...
            mpoints[state](pointInd,player) = val;
          }
        }
        cout << endl;
        ++pointInd;
